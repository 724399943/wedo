...
<body>
	...
	<div class="content" id="content">
		<header class="head">
			...
			<h1 class="y-confirm-order-h1"><?php echo (L("_WAP_AGENT_MALL_")); ?></h1>
		</header>
		<div class="main">
					...
					<ul>
						<li v-for="(item,index) in goodsList">
							<a :href="'goodsDetail?goods_id='+item.id">
								...
								<p><span class="name">{{item.goods_name}}</span><span class="num"><?php echo (L("_COMMON_SALES_NUMBER_")); ?>{{item.sale_number}}</span></p>
								<i>{{item.goods_price}}<?php echo (L("_WAP_POINTORDER_POINTS_")); ?></i>
							</a>
						</li>
					</ul>
				</div>
		...
		var vm = new Vue({
			...
			methods : {
				getGoods : function(){
					var that = this;
					$.ajax({
						url: "<?php echo U("Point/pointGoods");?>",
						type: "POST",
						dataType: "json",
						data: {
							page : that.page
