...
<body>
	...
	<div class="content" id="content">
		<header class="head">
			...
			<h1 class="y-confirm-order-h1"><?php echo (L("_WAP_BIDDING_BIDDINGRECORD_")); ?></h1>
			<a :href="'/Bidding/platformBiddingRecord?bidding_type='+bidding_type+ '&&' +'tab='+tab " class="mail"><?php echo (L("_WAP_BIDDING_WEDOBIDDINGRECORD_")); ?></a>
		</header>
		<div class="main">
			<div class="bidd_wrap">
						...
						<div class="sel_goodsm">
							<ul>
								<li v-for="(item,index) in goodsList">
									...
									<b class="sb" v-if="item.status == '0'"><?php echo (L("_WAP_BIDDING_PENDINGREVIEW_")); ?></b>
									<b class="sb" v-else-if="item.status == '1'"><?php echo (L("_WAP_BIDDING_BIDDINGSUCCESSFUL_")); ?></b>
									<b class="sb" v-else><?php echo (L("_WAP_BIDDING_BIDDINGAPPROVALFAILED_")); ?></b>
								</li>
							</ul>
						</div>
					</div>
				...
				<div class="integral_goods" v-else-if="tab == 2">
					...
					<a href="<?php echo U('Bidding/toBiddingAgent');?>" class="widup"><?php echo (L("_WAP_BIDDING_APPLYFORBIDDING_")); ?></a>
				</div>
				<!-- 广告位竞价记录 -->
	...
	<script type="text/javascript">
		var vm = new Vue({
			...
			data : {
				goodsList : [],
				page : 1,
				bidding_type : <?php echo ($_GET['bidding_type']); ?>,
				goscroll : 1,
			...
			methods : {
				getGoods : function(){
					var that = this;
					$.ajax({
						url : "<?php echo U("Bidding/biddingRecord");?>",
						type : "POST",
						dataType : "json",
						data : {
							page : that.page,
