...
var sendPrivateText = function(userInfo, to) {
    ...
    msg.set({
        ...
        success: function (id, serverMsgId) {
            ...
            time(to);
            var html = "<div class='em-widget-right'><div class='em-widget-msg-wrapper'><i class='icon-corner-right'><img src="+ext['user'].headimgurl+"></i><div class='em-widget-msg-container em-widget-msg-txt'>"+ test +"</div></div></div>";
            $('#em-widgetBody .em-widget-chat').hide();
            $('#chat_'+to).append(html).show();
            saveChat(to, 1, test, '');
            // console.log('发送成功!');
        }
    });
...
var reTextmsg = function(message){
    ...
    var ext = message.ext.user;
    var html = "<div class='em-widget-left'><div class='em-widget-msg-wrapper'><i class='icon-corner-left'><img src="+ext['headimgurl']+"></i><div class='em-widget-msg-container em-widget-msg-txt'>"+msg+"</div></div></div>"
    time(message.from);
    $('#em-widgetBody .em-widget-chat').hide();
    $('#em-widgetBody').find('#chat_'+message.from).append(html).show();
    $("#Talker").text(ext.nickname);
    var talker = "<li data-id="+ message.from +" data-nickname="+ ext.nickname +"><div class='headImgbox'><img src="+ext.headimgurl+"></div><span class='name'>"+ext.nickname+"</span></li>";
    var isExists = false,
        that = $("#Jcontact li");
    for (var i = 0; i < that.length; i++) {
        var nickname = that.eq(i).data('nickname');
        if ( nickname == ext.nickname ) {
            isExists = true;
            break;
        }
    }
...
var time = function(id){
    ...
    if ( $('#em-widgetBody').find('#chat_'+id).length ) {
        $('#em-widgetBody').find('#chat_'+id).append(timecont);
    } else {
        $('#em-widgetBody').append('<div class="em-widget-chat" id="chat_'+id+'">'+timecont+'</div>');
    }
}
