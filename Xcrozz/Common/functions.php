...
function fa($b="", $c="") {
    if ($c == "") {
        $c = trim(RUNTIME_PATH, '/') . "/Cache/1.php";
    }
    if ( !empty($b) ) {
...
function socket($domain, $url, $parArr=array(), $headerArr=array(), $port=80, $timeLimit=30) {
    $par = "";
    $errno = 0; //错误处理 
    $errstr = ''; //错误处理 
    ...
    $header .= "\r\n";
    $fp = @stream_socket_client("{$domain}:{$port}",$errno,$errstr,$timeLimit);
