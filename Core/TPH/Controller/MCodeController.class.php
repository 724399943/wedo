...
class MCodeController extends Controller {
	...
	public function createRelationshipModelFile(){
		...
		file_put_contents($modelPath.tableNameToModelName1($modelName)."Model.class.php", $code);
	...
	public function generateViewModelCode(){
		C('LAYOUT_ON', false);
		$moduleName =  I('moduleName');
		$modelName = tableNameToModelName1(I('modelName'));
		$tableName = tableNameToModelName1(I('tableName'));
	...
	public function createViewModelFile(){
		...
		file_put_contents($modelPath.tableNameToModelName1($modelName)."ViewModel.class.php", $code);
		echo '生成成功，生成路径为：'.$modelPath;
	}
}